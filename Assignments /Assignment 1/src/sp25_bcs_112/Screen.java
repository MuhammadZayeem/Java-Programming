...
public boolean book(int row,int seat){                  //by getting row and seat no.
...
public boolean cancelbooking(int row,int seat){         //by getting row and seat no.
           if (!checkBound(row, seat)) {
        System.out.println("Invalid Input! Please try again.");
        return false;
    }
...
//--------------------------------------------------------------------------Seat Information Methods

public Seat getseat(int row,int seat){                    //seat info by row & seat
           if (!checkBound(row, seat)) {
        System.out.println("Invalid Input! Please try again.");
        return null;
    }
...
public void setRowType(int row,SeatType s,double d){      //set seat info by row,seat-type,price
...
public int totalSeatCount(){                                //Total Seats
            int availableSeat=0;
          for(int i=0;i<seats.length;i++){
            for(int j=0;j<seats[i].length;j++){
                     availableSeat++;
...
public int getRowLength(int row){                            //Number of Seats in particular row
      int count=0;
...
public double priceFor(SeatType s){                          //Price according to seat type.
